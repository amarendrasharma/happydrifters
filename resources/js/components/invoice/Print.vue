<template>
    <div class="card card-body bg-white" v-if="order != null">   
        ...
        <div class="container rounded" style="position: relative; border: 1px dashed #e5e5e5" v-if="2 == layout">
            <div class="row">
                <div class="col-md-12">
                    <div ref="printTable">
                        <div class="px-3 py-3 w-100">
                            ...
                            <table class="w-100 mb-1 p-2" style="border: 1px solid #222">
                                <thead>
                                    <tr class="small"> 
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="border p-1">Sl. No.</th> 
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="border p-1">Description of Item</th> 
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="border p-1">HSN Code</th> 
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="text-xs-center text-center border p-1">Qnty.</th>
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="text-right border p-1">Rate</th>
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="text-right border p-1">Amount</th>
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="text-right border p-1">Discount</th>
                                        <th colspan="2" style="font-weight:500;font-size:80%;" class="text-center border p-1">SGST</th>
                                        <th colspan="2" style="font-weight:500;font-size:80%;" class="text-center border p-1">CGST</th>
                                        <th rowspan="2" style="font-weight:500;font-size:80%;" class="text-right border p-1">Total Amount</th>
                                    </tr>
                                    <tr class="small">
                                ...
                                <tbody class="mb1 small">
                                    <tr v-for="(item, index) in order.invoicedetails" :key="item.id" class="small"> 
                                        ...
                                        <td class="border-right p-1 text-xs-center text-center"> {{item.product_quantity}}</td>
                                        <td class="border-right p-1 text-right">{{ item.product_price }}</td>
                                        <td class="border-right p-1 text-right">{{ item.base_amount | formattedCurrency }}</td>
                                        <td class="border-right p-1 text-right">{{  item.discount_amount | formattedCurrency }}</td>
                                        <td class="border-right p-1 text-right">{{ item.sgst_per }}</td>
                                        <td class="border-right p-1 text-right">{{ item.sgst_value | formattedCurrency }}</td>
                                        <td class="border-right p-1 text-right">{{ item.cgst_per }}</td>
                                        <td class="border-right p-1 text-right">{{ item.cgst_value | formattedCurrency }}</td>
                                        <td class="border-right p-1 text-right">{{ item.total_amount | formattedCurrency }}</td>
                                    </tr> 
                                    ...
                                    <tr class="small">
                                        ...
                                        <td class="border text-right p-1">
                                            {{ order.total_gst_amount /2 | formattedCurrency }}
                                        </td>
                                        <td class="border text-right p-1"></td>
                                        <td class="border text-right p-1">
                                            {{ order.total_gst_amount /2 | formattedCurrency }}
                                        ...
                                        <td class="border text-right p-1">
                                            ...
                                            <div>
                                                {{ parseFloat(order.total_amount) | formattedCurrency }}
                                            </div>
                                        </td>
                                    </tr>
                                    <tr class="small">
                                    ...
                                    <tr class="small">
                                        <td colspan="2" class="px-1 py-2">
                                            <p class="text-center mb-2">Cerified that the particulars given above are true &amp; Correct</p>
                                            <p class="text-center mb-0 title fw-bold font-weight-bold">For: HAPPY DRIFTERS LLP</p>
                                        </td> 
                                        <td colspan="10" class="border text-center">Rupees {{parseFloat(order.total_amount) | toWords | titleCase}} only</td> 
                                    </tr>
                                    <tr class="small">
        ...
        getInvoiceDetails() {
            ...
            return axios.get(url)
                .then(response => {    
                    ...
                    EventBus.$emit('invoice-amount', response.data.data.total_rounded_amount_payable)
                })
		},
